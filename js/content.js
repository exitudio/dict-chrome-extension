...
function findAndClickSkipAds() {
    var checkCount = 0
    var checkingId = setInterval(function() {
        console.log("check there is any ads");
        checkCount++;
        if (checkCount > 4) {
            return clearInterval(checkingId);
        }
        if (document.getElementsByClassName("ytp-ad-preview-container countdown-next-to-thumbnail")[0]) {
            clearInterval(checkingId);
            document.getElementsByClassName("video-stream html5-main-video")[0].playbackRate=10
            var count = 0;
            var id = setInterval(function() {
                console.log('check skip ads')
                count++;
                if (count>10) {
                    clearInterval(id)
                }
...
if (window.location.hostname == "www.youtube.com") {
    ...
    window.onpopstate = findAndClickSkipAds;
}
